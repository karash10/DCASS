...
docker compose --profile training run dcass-train-gan \
  python -u scripts/train_gan.py \
    --resume /app/models/gan/epoch_025.pt
```
...
python scripts/add_wikipedia_to_index.py    # Text index
python scripts/audio_step2_build_index.py   # Audio index
...
# With visualization
python scripts/demo_full_loop.py --visualize
```
...
python scripts/train_gan.py --wgan-gp

# Or reduce warden learning rate
python scripts/train_gan.py --lr-warden 1e-4
```
...
python scripts/train_rl.py --lambda-stealth 50.0

# Or increase learning rate
python scripts/train_rl.py --lr 1e-3
```
...
```bash
# Reduce batch size
python scripts/train_gan.py --batch-size 16
...
python scripts/train_gan.py --device cuda

# Increase batch size (if enough memory)
python scripts/train_gan.py --batch-size 64

# Reduce logging frequency
python scripts/train_rl.py --log-interval 50

# Use multiple workers
python scripts/train_gan.py --num-workers 4
```
...
python scripts/train_gan.py --epochs 50 --batch-size 32
