...
- `scripts/train_gan.py` — 88 lines
- `scripts/train_rl.py` — 84 lines
- `scripts/generate_traffic_data.py` — Ready
...
| **Alice (Sender)** | ✅ Complete | `scripts/run_sender.py` (383 lines) |
| **Bob (Receiver)** | ✅ Complete | `scripts/run_receiver.py` (296 lines) |
...
1. **Generate synthetic traffic data:**
   ```bash
   python scripts/generate_traffic_data.py --num-sessions 2000 --output data/behavioral/human_traffic.json
   ```

2. **Train GAN scheduler:**
   ```bash
   python scripts/train_gan.py --data data/behavioral/human_traffic.json --epochs 50
   ```
