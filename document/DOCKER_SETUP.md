...
```bash
docker-compose run dcass-sender python scripts/run_sender.py --mode train --episodes 1000
```
...
1. **dcass-sender** (Alice):
   - Runs `scripts/run_sender.py`
   - Sends media sequences using RL scheduling
   - Writes packet metadata to `/app/shared_channel`

2. **dcass-receiver** (Bob):
   - Runs `scripts/run_receiver.py`
   - Monitors `/app/shared_channel` for incoming packets
...
```bash
docker-compose run dcass-sender python scripts/run_sender.py \
    --mode train \
    --episodes 1000
```
...
```bash
docker-compose exec dcass-receiver ls -la /app/shared_channel
docker-compose exec dcass-sender ls -la /app/shared_channel
```
